...
var Action = class {
     ...
     * @param {?String} wm_class // may be undefined when user does not set enough parameters
     * @param {?String} title // may be undefined when user does not set enough parameters
     * @param {Mode} mode
    ...
    constructor(app, shortcut_bare, layers, mode, command = "", wm_class = "", title = "") {
        ...
        this.wmFn = null
        this.titleFn = null;
        [this.wm_class, this.wmFn] = this._allow_regex(this.wm_class);
    ...
    is_conforming(window) {
        let [command, wmNeedle, wmFn, title, titleFn] = [this.command, this.wm_class, this.wmFn, this.title, this.titleFn];
        ...
        if (seen) {
            if (!seen.has_focus()) {
                ...
                if (mode.get(Mode.SWITCH_BACK_WHEN_FOCUSED)) {
                    const window_workspace = window.get_workspace();
                    const window_list = windows.filter(w => w.get_workspace() === window_workspace && w !== window)
